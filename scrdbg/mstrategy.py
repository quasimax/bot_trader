    ...
    def fsm_wait_order_fill(self, arg):
        ...
        if self.current_order().newClientOrderId == self.report['c'] and "FILLED" == self.report['X']:
            ...
            if "USDT" == cmsn_asset:
                ...
                self.orders[self.step].quantity = self.orders[self.step].quantity - cmsn_symb
                self.orders[self.step].quantity = self.round_at_step(self.orders[self.step].quantity, self.stepSize)
                self.send_orders_change("Комиссия в USDT, количество в ордере уменьшено")
                self.h_to_log("Correction order {}: new qantity {}".format(self.orders[self.step].newClientOrderId, self.orders[self.step].quantity))
            elif "BNB" != cmsn_asset:
                self.orders[self.step].quantity = self.orders[self.step].quantity - float(self.report['n'])
                self.orders[self.step].quantity = self.round_at_step(self.orders[self.step].quantity, self.stepSize)
