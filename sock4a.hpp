...
public:
 ...
 request(command_type cmd, const std::string& user_id)
        : version_(version),
          command_(cmd),
          port_high_byte_(0),
          port_low_byte_(0),
          ...
          null_byte_(0)
{
 ...
 boost::array<boost::asio::const_buffer, 8> buffers() const
 {
    ...
    {
      {
        ...
        boost::asio::buffer(&null_byte_, 1),     // for socks4a
 ...
 boost::array<boost::asio::const_buffer, 8> buffers(const boost::asio::ip::tcp::endpoint& endpoint, const std::string& user_id)
 {
      // Convert port number to network byte order.
      unsigned short port = endpoint.port();
      port_high_byte_ = (port >> 8) & 0xff;
 ...
 boost::array<boost::asio::const_buffer, 8> buffers_ep(const boost::asio::ip::tcp::endpoint& endpoint)
  {
      // Convert port number to network byte order.
      unsigned short port = endpoint.port();
      port_high_byte_ = (port >> 8) & 0xff;
